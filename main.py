...
from core import is_victory, load_level, move_level, solve_level_bfs
from game_render import BG_COLOR, CELL_SIZE, draw_level, draw_text
...
def run_menu(start_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    def activate_selected() -> Optional[dict]:
        ...
        if item_type == "edit":
            return {"action": "edit", "dir": current_dir, "menu_dir": current_dir}
        ...
        if item_type == "level":
            return {"action": "play", "path": item_path, "menu_dir": current_dir}
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    menu_items = build_menu_items(current_dir)
                    selected_index = max(0, min(selected_index, len(menu_items) - 1)) if menu_items else 0
                ...
                if menu_items:
                    ...
                    for draw_i, item_index in enumerate(range(start_index, end_index)):
                        ...
                        item_rect = pygame.Rect(list_left + 12, y - 3, list_width - 24, item_height - 2)
                    ...
                    if clicked_index is not None:
                        ...
                        is_double_click = clicked_index == last_click_index and now_ms - last_click_ms <= double_click_threshold
        ...
        pygame.draw.rect(screen, (34, 38, 46), (list_left, list_top, list_width, list_height), border_radius=12)
        pygame.draw.rect(screen, (70, 78, 95), (list_left, list_top, list_width, list_height), width=2, border_radius=12)

        if not menu_items:
            draw_text(screen, "当前目录为空。你可以在此目录打开编辑器创建关卡。", list_left + 20, list_top + 30, 28)
        else:
            draw_text(screen, f"当前可选项：{len(menu_items)} 个", list_left + 20, list_top + 16, 24)
            for draw_i, item_index in enumerate(range(start_index, end_index)):
                y = start_y + draw_i * item_height
                ...
                draw_text(screen, f"{item_index + 1:>3}.", list_left + 24, y, 23, color=text_color)
                draw_text(screen, item["label"], list_left + 90, y, 23, color=text_color)

            draw_text(screen, f"当前选中：{menu_items[selected_index]['label']}", list_left + 20, list_top + list_height - 32, 22, color=(180, 220, 180))
...
def run_level(level_path: Path) -> None:
    ...
    win_h = level.height * CELL_SIZE + WINDOW_EXTRA_HEIGHT
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_h:
                    ...
                    last_solution_text = format_solution(solution)
                    continue
        ...
        draw_text(screen, "方向键移动，R重开，H求解，ESC返回", 14, panel_y + 45, 22)
        ...
        if won:
            ...
            draw_text(screen, "过关规则：所有目标都必须由对应角色占据", 14, panel_y + 108, 22, color=(200, 210, 220))
